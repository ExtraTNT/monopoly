...
public class BuyPropertyUI extends VBox {
    ...
    public BuyPropertyUI() {
        super(5);
        VBox buyPropertyScreen = new VBox();

        Label buyPropertyScreenTitle = new Label("Willst du dieses Feld Kaufen?");
        Label buyPropertySpaceName = new Label(Game.getInstance().getBoard().getFieldByIndex(Game.getInstance().getCurrentPlayer().getPosition()).getName());
